// Shared branded card wrapper for transactional emails sent via
// src/lib/email.ts (sendEmail). Matches the look of the Supabase Auth
// templates (supabase/templates/*.html) and the earlier welcome-email /
// follow-up email — logo header, white rounded card, brand green CTA,
// muted footer. Keep content ASCII-only (no em dashes, curly quotes, or
// (c) symbols) for consistency with the rest of this app's email content.
export function renderEmailCard({
  title,
  heading,
  bodyHtml,
  cta,
  footerNote,
}: {
  title: string;
  heading: string;
  bodyHtml: string;
  cta?: { text: string; href: string };
  footerNote?: string;
}): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>${title}</title>
</head>
<body style="margin:0; padding:0; background-color:#f4fffa; font-family:Arial, Helvetica, sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4fffa; padding:40px 16px;">
    <tr>
      <td align="center">
        <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="max-width:480px; width:100%; background-color:#ffffff; border-radius:24px; box-shadow:0 4px 24px rgba(0,0,0,0.06); overflow:hidden;">

          <tr>
            <td align="center" style="padding:40px 40px 24px;">
              <table role="presentation" cellpadding="0" cellspacing="0">
                <tr>
                  <td valign="middle" style="padding-right:8px;">
                    <img src="https://www.billfixr.com/logo-icon-green.png" width="28" height="28" alt="BillFixr" style="display:block;" />
                  </td>
                  <td valign="middle">
                    <span style="font-size:20px; font-weight:700; color:#0f7545;">Bill</span><span style="font-size:20px; font-weight:700; color:#eaa93f;">Fixr</span>
                  </td>
                </tr>
              </table>
            </td>
          </tr>

          <tr>
            <td style="padding:0 40px 8px;">
              <h1 style="margin:0; font-size:22px; line-height:1.3; color:#003322; font-weight:700; text-align:center;">
                ${heading}
              </h1>
            </td>
          </tr>
          <tr>
            <td style="padding:16px 40px 0;">
              ${bodyHtml}
            </td>
          </tr>
${
  cta
    ? `
          <tr>
            <td align="center" style="padding:32px 40px 8px;">
              <table role="presentation" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center" style="border-radius:999px; background-color:#0f7545;">
                    <a href="${cta.href}"
                       style="display:inline-block; padding:14px 40px; font-size:14px; font-weight:700; color:#ffffff; text-decoration:none; border-radius:999px;">
                      ${cta.text}
                    </a>
                  </td>
                </tr>
              </table>
            </td>
          </tr>`
    : ""
}
          <tr>
            <td style="padding:32px 40px 0;">
              <div style="border-top:1px solid #ebebeb;"></div>
            </td>
          </tr>

          <tr>
            <td style="padding:20px 40px 40px;">
${footerNote ? `              <p style="margin:0; font-size:12px; line-height:1.6; color:#a6b1bb; text-align:center;">${footerNote}</p>\n` : ""}              <p style="margin:${footerNote ? "16px" : "0"} 0 0; font-size:11px; color:#a6b1bb; text-align:center;">
                Copyright 2026 BillFixr Technologies. All Rights Reserved.
              </p>
            </td>
          </tr>

        </table>
      </td>
    </tr>
  </table>
</body>
</html>`;
}

// Standard body paragraph style used inside bodyHtml above.
export function emailParagraph(text: string): string {
  return `<p style="margin:0; font-size:14px; line-height:1.6; color:#4d6276; text-align:center;">${text}</p>`;
}
